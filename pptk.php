...
if(isset($_POST['tambah'])){
	$pptk=$_POST['pptk'];
	$paket=$_POST['paket'];
	$q=mysqli_query($mycon,"insert into simoni_pptk values(null,'$pptk',$paket)");
	if($q){
		?><script>alert('SUKSES!\n\nData berhasil ditambahkan.');</script><?php
	}else{
		?><script>alert('GAGAL!\n\nData gagal ditambahkan.');</script><?php
	}
	?><script>location.href='?menu=pptk';</script><?php
}
?>	
	<!-- Page content -->
	<div id="page-content">
		...
		<div class="content-header">
			<div class="header-section">
				<h1>
					<i class="gi gi-bullhorn"></i>Kelola PPTK Paket Pekerjaan<br><small>Nama PPTK &amp; Nama Paket Pekerjaan</small>
				</h1>
			</div>
		</div>
			...
			<div class="col-md-12">
				...
				<div class="block">
					...
					<div class="block-title">
						<h2><strong>Tambah</strong> PPTK Paket Pekerjaan</h2>
					</div>
					<!-- END Form Elements Title -->
					<?php
					if(isset($_GET['aksi'])){
						if($_GET['aksi']=='hapus'){
							$q=mysqli_query($mycon,"delete from simoni_pptk where pptkid=$_GET[id]");
							if($q){
					...
					<form method="post" enctype="multipart/form-data" class="form-horizontal form-bordered" >
						<div class="form-group">
							<label class="col-md-3 control-label" for="pptk">Nama PPTK</label>
							<div class="col-md-9">
								<select id="pptk" name="pptk" class="form-control" size="1">
									<?php
									$q=mysqli_query($mycon,"select * from simoni_user where level='PPTK'");
						...
						<div class="form-group">
							...
							<div class="col-md-9">
								<select id="paket" name="paket" class="select-chosen" data-placeholder="Pilih Paket.." size="1">
									<?php
									$q=mysqli_query($mycon,"select * from simoni_paket where tahun=year(now()) and paketid not in(select paketid from simoni_pptk)");
									while($h=mysqli_fetch_array($q)){
									?>
		...
		<div class="block full">
			...
			<div class="table-responsive">
				<table id="dtable" class="table table-vcenter table-condensed table-bordered">
					...
					<tbody>
						<?php
						$q=mysqli_query($mycon,"select * from simoni_pptk");
						while($h=mysqli_fetch_array($q)){
						...
						<tr>
							...
							<td class="text-center"><?php echo $pptk['nama']; ?></td>
							...
							<td class="text-center">
								<div class="btn-group">
									<a href="?menu=pptk&id=<?php echo $h['pptkid']; ?>&aksi=hapus" data-toggle="tooltip" title="Delete" class="btn btn-xs btn-danger"><i class="fa fa-times"></i> Hapus</a>
								</div>
							</td>
						</tr>
						<?php
